    ...
    <div class="card-body">
        <div class="row">
            <div class="col-md-6">
                ...
                <strong>Status:</strong> <span class="badge bg-primary">{{ Str::title(str_replace('_', ' ', $order->status)) }}</span>
            </div>
            <div class="col-md-6">
        ...
        @if ($order->status == 'waiting_payment')
            <div class="alert alert-warning">
                ...
                <p>Untuk melanjutkan pesanan, silakan transfer sejumlah <strong>Rp {{ number_format($order->total_price, 0, ',', '.') }}</strong> ke rekening berikut:</p>
            ...
            <form action="{{ route('order.upload_proof', $order) }}" method="POST" enctype="multipart/form-data" class="mt-3">
                @csrf
                <div class="mb-3">
        ...
        @elseif ($order->status == 'waiting_confirmation')
            <div class="alert alert-info">
                ...
                <p>Terima kasih! Bukti pembayaran Anda telah kami terima dan akan segera kami periksa. Mohon tunggu konfirmasi dari admin.</p>
                <hr>
                <strong>Bukti yang Anda upload:</strong><br>
                <img src="{{ asset('storage/' . $order->payment_proof) }}" alt="Bukti Pembayaran" class="img-fluid mt-2" style="max-width: 300px;">
            </div>
        @elseif ($order->status == 'confirmed')
             <div class="alert alert-success">
                <h4 class="alert-heading">Pesanan Dikonfirmasi!</h4>
                <p>Pembayaran Anda telah kami konfirmasi. Pesanan Anda akan kami siapkan sesuai tanggal acara. Terima kasih telah memesan!</p>
            </div>
        @endif
    </div>
</div>
