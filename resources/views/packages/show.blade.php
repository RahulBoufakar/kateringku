...
@section('content')
    <div class="container mt-5">
        <div class="row">
            <div class="col-md-6">
                <img src="{{ $package->image_url ? asset('storage/' . $package->image_url) : 'https://via.placeholder.com/600x400' }}"
                    class="img-fluid rounded shadow-sm" alt="{{ $package->name }}">
            </div>
            <div class="col-md-6">
                ...
                <ul>
                    @foreach ($package->menuItems as $item)
                        <li>{{ $item->nama_menu }} ({{ $item->kategori }})</li>
                    @endforeach
                </ul>

                <div class="d-grid gap-2 mt-4">
                    <button class="btn btn-primary btn-lg" data-bs-toggle="modal"
                        data-bs-target="#orderPackageModal-{{ $package->id }}">
                        Pesan Paket Ini
                    </button>
                </div>
            </div>
    ...
    <div class="modal fade" id="orderPackageModal-{{ $package->id }}" tabindex="-1" aria-labelledby="orderModalLabel"
        aria-hidden="true">
